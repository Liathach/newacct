    ...
    if (e.target.nodeName === 'BUTTON') {
        ...
        if (e.target.textContent === 'Add Account') {
            ...
            if (idAcctName.value.length > 0 && idInitDeposit.value.length > 0) {
                ...
                i++;
                idAccountStatusMsg.innerText = `The total balance of all accounts is $${AcctControls.totalAcctBalance()}`;
                clearFields();
            }
            ...
            idAccountStatusMsg.innerText = `The lowest value account and balance is: ${AcctControls.lowestAcctBalance()}`;
        
        } else if (e.target.textContent === "Highest Account Balance") {

            idAccountStatusMsg.innerText = `The highest value account and balance is: ${AcctControls.highestAcctBalance()}`;
